...
const bind=()=>{
 document.querySelectorAll('[data-cursor],[data-cursor-type],a,button,[role="button"],.project-tile,.archive-card,.project-screen,.observatory-stage,.play-card,.arcade-card,.physics-box').forEach(el=>{
  if(el.matches('.cursor-ball,.cursor-label')||el.dataset.rayCursorBound==='1')return;
  el.dataset.rayCursorBound='1';
  el.addEventListener('pointerenter',()=>setActive(el),{passive:true});
  el.addEventListener('pointerleave',()=>clear(el),{passive:true});
 });
 // Headings get the existing CSS glitch treatment without changing their copy.
 if(!reduce){
  document.querySelectorAll('h1,h2,h3,.display-title,.section-title,.signal-title').forEach(el=>{
...
addEventListener('pointermove',e=>{
 ...
 core?.setPointer({x:e.clientX,y:e.clientY,vx:(e.clientX-state.tx)/dt*16,vy:(e.clientY-state.ty)/dt*16});
 // Wake only the surface currently under the pointer.
 const target=e.target?.closest?.('.local-interaction,.spotlight,.archive-art,.tile-art,.lab-visual,.game-screen,.physics-box,.project-screen,.observatory-stage,.play-card');
 if(target){const r=target.getBoundingClientRect();target.style.setProperty('--mx',`${((e.clientX-r.left)/Math.max(1,r.width))*100}%`);target.style.setProperty('--my',`${((e.clientY-r.top)/Math.max(1,r.height))*100}%`)}
},{passive:true});
...
function makeTrail(x,y,vx,vy){
 ...
 if(now-lastTrail<16||state.speed<3.2)return;
 lastTrail=now;
 const dot=document.createElement('i');dot.className='cursor-comet';
 const size=Math.min(9,2+state.speed*.2),angle=Math.atan2(vy,vx),stretch=Math.min(4,1+state.speed*.18);
 dot.style.width=`${size*stretch}px`;dot.style.height=`${Math.max(1.4,size*.42)}px`;
 dot.style.left=`${x}px`;dot.style.top=`${y}px`;dot.style.transform=`translate(-50%,-50%) rotate(${angle}rad)`;
...
const loop=()=>{
 ...
 const capped=Math.min(28,state.speed);
 const stretch=1+Math.min(.42,capped*.021);
 const squash=1/Math.sqrt(stretch);
 const angle=state.speed>.05?Math.atan2(state.vy,state.vx):0;
 ball.style.transform=`translate3d(${state.x}px,${state.y}px,0) translate(-50%,-50%) rotate(${angle}rad) scale(${stretch},${squash})`;
